...
TCA6424A::TCA6424A(uint8_t address, uint8_t reset) {
  _deviceAddr = address;
  _resetLine = reset;
}
...
/* ------------------------------------------------------- */
void TCA6424A::begin() {
  pinMode(_resetLine, OUTPUT);
  digitalWrite(_resetLine, HIGH);
  ...
  Wire.beginTransmission(_deviceAddr); // start data sending
...
void TCA6424A::writePort(uint8_t port, uint8_t data) {
  ...
  Wire.beginTransmission(_deviceAddr);   // start I2C addr
